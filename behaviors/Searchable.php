<?php namespace Octobro\Searchable\Behaviors;
...
class Searchable extends ExtensionBase
{
    ...
    protected function getDatabaseDriver() {
        $key = $this->model->connection ?: Config::get('database.default');
    ...
    {
        if (array_key_exists('columns', $this->model->searchable)) {
            $driver = $this->getDatabaseDriver();
            $prefix = Config::get("database.connections.$driver.prefix");
            $columns = [];
            foreach($this->model->searchable['columns'] as $column => $priority){
                $columns[$prefix . $column] = $priority;
            }
            return $columns;
        } else {
            return DB::connection()->getSchemaBuilder()->getColumnListing($this->model->table);
        }
    }
    ...
    {
        if (array_key_exists('groupBy', $this->model->searchable)) {
            return $this->model->searchable['groupBy'];
        }
    ...
    {
        return $this->model->searchable['table_columns'];
    }
    ...
    {
        return array_get($this->model->searchable, 'joins', []);
    }
    ...
    protected function makeGroupBy(Builder $query)
    {
        if ($groupBy = $this->getGroupBy()) {
            ...
            if ($driver == 'sqlsrv') {
                ...
                $columns = $this->model->getTable() . '.' . ($this->model->primaryKey ?: 'id');
            }
    ...
    protected function mergeQueries(Builder $clone, Builder $original) {
        $tableName = DB::connection($this->model->connection)->getTablePrefix() . $this->model->getTable();
        if ($this->getDatabaseDriver() == 'pgsql') {
            $original->from(DB::connection($this->model->connection)->raw("({$clone->toSql()}) as {$tableName}"));
        } else {
            $original->from(DB::connection($this->model->connection)->raw("({$clone->toSql()}) as `{$tableName}`"));
        }
